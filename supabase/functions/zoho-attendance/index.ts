...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Parse query parameters for attendance
    const url = new URL(req.url);
    const date = url.searchParams.get('date') || new Date().toISOString().split('T')[0];
    const empId = url.searchParams.get('empId');
    const emailId = url.searchParams.get('emailId');
    const erecno = url.searchParams.get('erecno');
    ...
    const zohoApiUrl = new URL(`${tokens.people_base_url}/people/api/attendance/getAttendanceEntries`);
    zohoApiUrl.searchParams.set('date', formattedDate);
    
    if (empId) zohoApiUrl.searchParams.set('empId', empId);
    else if (emailId) zohoApiUrl.searchParams.set('emailId', emailId);
    else if (erecno) zohoApiUrl.searchParams.set('erecno', erecno);

    console.log('Calling Zoho Attendance API:', zohoApiUrl.toString());
